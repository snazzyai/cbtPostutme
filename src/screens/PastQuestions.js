...
class PastQuestions extends Component {
  ...
  componentDidMount() {
    console.log("here");
    //   console.log(`${base_url}/putme_schools/`)
    fetch(`${variables.base_url}putme_schools/`)
    ...
    if (result.action !== Share.sharedAction) {
      ...
      new Promise(resolve => {
        if (AsyncStorage.getItem("shared") !== null) {
          resolve(
            setTimeout(() => this.props.navigation.navigate("Download"), 7000)
          );
        } else {
        }
      });
    ...
    return (
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.container}>
          ...
          {this.state.loaded && QuestionList}
        </View>
      </ScrollView>
    );
  }
